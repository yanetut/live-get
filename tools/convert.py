...
def covj(xmlfp, jsonfp, **opt):
    ...
    try:
        data = xmlfp.read()
        data = data.replace("</i>", "").replace('<d p=\"', '')
        ...
        del(data)
        jsonfp.write(json.dumps({'code': 0, 'data': dmp},
...
def covx(jsonfp, xmlfp, **opt):
    try:
        ...
        id = opt.get("id", "")
        xmlfp.write(xmlhead0+opt.get("chatserver", "domain")+xmlhead1+id +
                    xmlhead2+"1000"+xmlhead3+"1000"+xmlhead4+str(len(dmp))+xmlhead5)
        for danmaku in dmp:
            ...
            if len(danmaku.get("color", "#ffffff")) == 4:
                color = str(int(danmaku.get("color")[1:2]+danmaku.get("color")[1:2]+danmaku.get("color")[
                            2:3]+danmaku.get("color")[2:3]+danmaku.get("color")[3:4]+danmaku.get("color")[3:4], 16))
            else:
                color = "16777215"
            post_time = str(danmaku.get("post_time", 0))
            comment = danmaku.get("text", "").encode("utf-8")
            tag = taghead+time+","+type+","+size+","+color + \
                ","+post_time+","+"0,0,0\">"+comment+tagtail
            xmlfp.write(tag)
        xmlfp.write(xmltail)
...
def main():
    ...
    if jindex == -1 and xindex != -1:
        try:
            covj(codecs.open(xpath, 'r', encoding="utf-8"),
                 codecs.open(xpath.split(".")[0]+".json", "w", encoding="utf-8"))
        except:
            print("Error:"+str(sys.exc_info()[1]))
            pass
        return
    if jindex != -1 and xindex == -1:
        try:
            covx(open(jpath), open(jpath.split(".")[
                 0]+".xml", "w"), id=jpath.split(".")[0])
        except:
            print("Error:"+str(sys.exc_info()[1]))
            pass
        return
